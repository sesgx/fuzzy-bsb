...
def _check_title_is_in_text(
    *,
    ...
    options = ["".join(x) for x in _window(text, size=window_size)]
...
class _PooledTitleIsInTextArgs(TypedDict):
...
def _pooled_check_title_is_in_text(
    args: _PooledTitleIsInTextArgs,
) -> bool:
    ...
    Examples:
        >>> _pooled_check_title_is_in_text(
        ...     {
        ...         "title": "regression tests for machine learning models: a systematic literature review",
    ...
    return _check_title_is_in_text(text=args["text"], title=args["title"])
...
class FuzzyBSBStudy:
    ...
    Examples:
        >>> s = FuzzyBSBStudy(id=1, title=" title. HERE ", text_content=" text. \n \r\n HERE ")
    ...
    def __init__(
        ...
        *,
        ...
        self.__text_content = _preprocess_text(text_content)
...
def fuzzy_bsb(
    studies: list[FuzzyBSBStudy],
) -> Iterator[tuple[FuzzyBSBStudy, list[FuzzyBSBStudy]]]:
    ...
    Examples:
        ...
        ... ]
        >>>
        >>> for study, references in fuzzy_bsb(studies):
    ...
    for study_index, study in enumerate(studies):
        with Pool() as p:
            func_args: list[_PooledTitleIsInTextArgs] = [
                {
